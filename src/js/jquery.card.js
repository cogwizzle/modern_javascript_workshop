...
const styles = {
  ...
  image: "width: 75px;height: 75px;border-radius: 50px;border: 1px solid black;",
...
jQuery.fn.card = function(options) {
  ...
  const card = jQuery(template.join('\n'));

  jQuery(this).append(card);

  jQuery(card).click(event => alert(options.abstract));
};

module.exports = jQuery.fn.card;
